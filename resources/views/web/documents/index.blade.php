       ...
       <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
         ...
         <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
          ...
          class="flex items-center justify-center text-white bg-sc_greeny hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
        ...
        {{-- Create Modal --}}
        @can('documents.create')
        <div id="createModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-scroll overflow-x-hidden fixed m-auto top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-auto max-h-[90vh]">
        ...
        <div class="overflow-x-auto">
         <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <thead class="text-sc_greener uppercase bg-sc_sandy dark:bg-gray-700 dark:text-gray-400">
           <tr>
          ...
          <tbody>
           ...
           <tr class="border-b text-sm text-gray-900 font-medium">
            <td class="px-4 py-3 text-xs">{{$doc->name}}</td>
            <td class="px-4 py-3">{{$doc->doc_type->name}}</td>
            <td class="px-4 py-3">{{$doc->date}}</td>
            <td class="px-4 py-3">{{$doc->number}}</td>
            <td class="px-4 py-3">{{$doc->sender}}</td>
            <td class="px-4 py-3">{{$doc->subject}}</td>
            <td class="px-4 py-3">{{$doc->area->name}}</td>
            <td class="px-4 py-3 " align="center"><a href="{{asset('storage/documents/'.$doc->document)}}" target="_BLANK">
